<?php
/**
 * 파일명 : contact_main_code.php
 * 내용 : CONTACT 메인 페이지 코드
 * 최초작성날짜 : 2024/03/18
 * 최초작성자 : 김민성
 * ------------------------------------
 * name       date        comment
 ...
 */
    ...
    try {
        ...
        $title_name = "Contact"; // Copy, CSV, Excel, Print 제목
    ...
    function getMain_List() {
        ...
        if (!empty($Main_list_arry)) {
            foreach ($Main_list_arry as $data) {
                $_db_CONTACT_SEQ = _check_var($data['CONTACT_SEQ']); // 시퀀스
                $_db_ORDER_NUMBER = _check_var($data['ORDER_NUMBER']); // 정렬값
                $_db_TITLE = _check_var($data['TITLE']); // 제목
                $_db_TITLE_EN = _check_var($data['TITLE_EN']); // 제목
                ...
                $url = "../board/contact_details.php?mode=MOD&{$query_string}";

                echo <<<TR
                            <tr>
                                <td class="simple_numbers"></td>
                                <td><a href="{$url}&seq={$_db_CONTACT_SEQ}">{$_db_TITLE}</a></td>
                                <td>{$_db_TITLE_EN}</td>
