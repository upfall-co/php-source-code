<?php
/**
 * 파일명 : orderHistory_nomember_result_code.php
 * 내용 : 주문상세정보 - 비회원 
 * 최초작성날짜 : 2023/08/08
 * 최초작성자 : 김민성
 * ------------------------------------
                 ...
                 SELECT M.PURCHASE_SEQ
                      ...
                      , (SELECT IFNULL(COUNT(*), 0)
                          ...
                          WHERE 1
                            ...
                            AND C.STATE_CD != '42') AS TOTAL_COUNT
                      , (SELECT IFNULL(SUM(C.PRICE), 0)
                           FROM PURCHASE_ORDER B, PURCHASE_OPTION C
                          WHERE 1
                            AND B.PURCHASE_SEQ = C.PURCHASE_SEQ
                            AND B.PURCHASE_SEQ = A.PURCHASE_SEQ
                            AND C.STATE_CD != '42') AS TOTAL_PRICE
                      , FORMAT((SELECT IFNULL(SUM(C.QUANTITY*C.PRICE), 0)
                                  FROM PURCHASE_ORDER B, PURCHASE_OPTION C
                                 WHERE 1
                                   AND B.PURCHASE_SEQ = C.PURCHASE_SEQ
                                   AND B.PURCHASE_SEQ = A.PURCHASE_SEQ
                                   AND C.STATE_CD != '42'), 0) AS TOTAL_PRICE_TEXT
