<?php
	...
	function executePartialRefundCode() {
		...
		$detail["price"] = $_SESSION['INIS']['PRICE']; // 취소요청 금액
		$detail["confirmPrice"] = $_SESSION['INIS']['CONFIRMPRICE']; // 남은금액
		$detail["currency"] = $_SESSION['INIS']['CURRENCY']; // 통화 (WON, USD)
		$detail["tax"] =  $_SESSION['INIS']['TAX'];  // 부가세
		$detail["taxfree"] = $_SESSION['INIS']['TAXFREE']; // 비과세
		...
		try {
			...
			if (strcmp("00", $resultMap["resultCode"]) == 0) {
				...
				$TID = @(in_array($resultMap["tid"] , $resultMap) ? $resultMap["tid"] : $_SESSION['INIS']['TID']); // 부분취소 - 취소 tid
